...
class AgilityFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_agility, container, false)
        gridLayout = view.findViewById(R.id.grid_layout_agility)

        heroRepository = AppFirebaseHeroRepository()
        attributeRepository = AppFirebaseAttributeRepository()
        val agility = "Agility"
        val agilityAttributeFirebaseId = attributeRepository.getAttributeFirebaseIdByName(agility)
        if (agilityAttributeFirebaseId != null){
            heroRepository.getHeroesByAttributeFirebaseId(agilityAttributeFirebaseId).
                    observe(viewLifecycleOwner, { agilityHeroes ->
                        populateGridLayout(agilityHeroes)
                    })
        }
